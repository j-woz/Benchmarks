...
def load_combined_dose_response(rename=True):
    ...
    df = pd.concat([df1, df2])
    logger.info(
        "Combined dose response data contains sources: {}".format(df["SOURCE"].unique())
    )
...
def load_drug_data(
    ncols=None, scaling="std", imputing="mean", dropna=None, add_prefix=True
):
    ...
    df_desc = (
        pd.merge(df_info[["ID", "Drug"]], df_desc, on="Drug")
        .drop("Drug", axis=1)
        .rename(columns={"ID": "Drug"})
    )
    df_fp = (
        pd.merge(df_info[["ID", "Drug"]], df_fp, on="Drug")
        .drop("Drug", axis=1)
        .rename(columns={"ID": "Drug"})
    )

    df_desc2 = load_drug_set_descriptors(
        drug_set="NCI60", usecols=df_desc.columns.tolist() if ncols else None
    )
    df_fp2 = load_drug_set_fingerprints(
        drug_set="NCI60", usecols=df_fp.columns.tolist() if ncols else None
    )
...
def cell_name_to_ids(name, source=None):
    ...
    hits1 = lookup(
        df1, name, "NCI60.ID", ["NCI60.ID", "CELLNAME", "Name"], match="contains"
    )
...
def load_drug_set_fingerprints(
    drug_set="Combined_PubChem",
    ncols=None,
    usecols=None,
    scaling=None,
    ...
    for fp in fps:
        ...
        df_cols = pd.read_csv(
            path, engine="c", sep="\t", nrows=0, skiprows=1, header=None
        )
        total = df_cols.shape[1] - 1
